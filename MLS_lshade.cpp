   ...
   //main loop
	...
	if(generation>50*(problem_size/10)*(problem_size/10)){
		if(bsf_fitness_generations[bsf_fitness_generations.size()-50*(problem_size/10)*(problem_size/10)]-bsf_fitness_generations[bsf_fitness_generations.size()-1]<epsilon) {
	    	...
	    	int max_num_DSGC=min(max_num_evaluations-nfes,(unsigned int)((max_num_evaluations/10)/pow((problem_size/10.0),1.0)));
		
	    	//local seach for the temporary best individual of population
	    	int t=DSCG_search(pop[sorted_array[0]],fitness[sorted_array[0]],problem_size,(max_region-min_region)/50,pow(10,-4.0*(problem_size/10.0)),0.75,max_num_DSGC);
	    	nfes+=t;
